...
本实验把正文段落规则限制为 `.article section > p:not(.eyebrow)`，避免把卡片内部的 `<p>` 一并命中。这样 `.problem-grid p` 可以使用普通选择器覆盖自己的样式，不需要通过 `!important` 提高优先级。

这也是长期维护 CSS 时更推荐的思路：**先缩小通用规则的作用范围，再考虑提高 specificity。**
